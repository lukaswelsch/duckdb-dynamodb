...
LogicalType AttributeTypeToLogical(const Aws::DynamoDB::Model::AttributeValue &av) {
	...
	if (av.GetType() == Aws::DynamoDB::Model::ValueType::BOOL)
...
void AppendItemToChunk(const Aws::Map<Aws::String, Aws::DynamoDB::Model::AttributeValue> &item,
                       ...
                       column_t row) {
	column_t output_count = MinValue<column_t>(projected_col_indices.size(), output.ColumnCount());
	for (column_t out_idx = 0; out_idx < output_count; out_idx++) {
		column_t schema_idx = projected_col_indices[out_idx];
		...
		case LogicalTypeId::VARCHAR:
			FlatVector::GetData<string_t>(vec)[row] = StringVector::AddString(vec, AttributeValueToString(av));
			break;
