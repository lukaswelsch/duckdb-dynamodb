...
static bool ExpectRowCount(Connection &con, const std::string &sql, idx_t expected) {
	...
	std::cout << (ok ? "✅ PASS" : "❌ FAIL") << " expected=" << expected << " got=" << actual << " | " << sql << "\n";
	return ok;
}
...
int main() {
	...
	// ══════════════════════════════════════════════════════════════════════
	RunQuery(con, "SELECT * FROM dynamodb_scan('orders', " + LOCAL + ", allow_full_scan=true) LIMIT 5",
	...
	RunQuery(con,
	         ...
	         " FROM dynamodb_scan('orders', " +
	             LOCAL +
	             ", allow_full_scan=true, parallel_segments=4)"
	...
	RunQuery(con,
	         ...
	         " FROM dynamodb_json('orders', " +
	             LOCAL + ", allow_full_scan=true) LIMIT 10",
	...
	// ══════════════════════════════════════════════════════════════════════
	{
		auto result = con.Query("SELECT * FROM dynamodb_scan('orders', " + LOCAL +
		                        ")"
		                        " WHERE nonIndexedCol = 'x'"); // no key predicate
		bool blocked = result->HasError() && result->GetError().find("allow_full_scan") != std::string::npos;
		std::cout << (blocked ? "✅ PASS" : "❌ FAIL") << " Full scan correctly blocked without allow_full_scan=true\n";
	}

	// ══════════════════════════════════════════════════════════════════════
	// TEST 8 — Replacement scan syntax
	// ══════════════════════════════════════════════════════════════════════
	RunQuery(con, "SELECT COUNT(*) FROM 'dynamodb://orders' -- requires allow_full_scan via SET",
